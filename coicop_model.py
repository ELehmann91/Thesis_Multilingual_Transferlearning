...
class predictor:
    ...
    def __init__(self,df,name_col,cat_col,url_col,label_col,lang,embedding_dim=300,label_dict3=coicop_5_3,label_dict4=coicop_5_4
        ...
        if cat_col is not None and url_col is not None:
            self.df['text'] = self.df[name_col].fillna('unknown') + ' <sep> ' + self.df[cat_col].fillna('unknown') + ' <sep> ' + self.df['url_text'].fillna('unknown') 
            ...
            self.df['text'] = self.df[name_col].fillna('unknown') + ' <sep> ' + self.df['url_text'].fillna('unknown') 
